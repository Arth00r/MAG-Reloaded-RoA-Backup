...
if attack == AT_NSPECIAL
{
    if hbox_num == 2
    {
        ...
        if hitbox_timer = 5
        {
            if (place_meeting(x + 50*spr_dir, y + 5, asset_get("par_block")) || place_meeting(x + 50 * spr_dir, y + 5, asset_get("par_jumpthrough")))
            {
                if count < 10
                {
                    var new_effect = spawn_hit_fx(x + 50*spr_dir, y, player_id.ex_nspecial_effect)
                    new_effect.spr_dir = spr_dir
                    new_effect.draw_xscale = draw_xscale
                    
                    var new_hitbox = create_hitbox(AT_NSPECIAL, 2, x + 50*spr_dir, y)
    ...
    if hbox_num == 1
    {
        if !(place_meeting(x + 25*spr_dir, y + 5, asset_get("par_block")) || place_meeting(x + 25 * spr_dir, y + 5, asset_get("par_jumpthrough")))
        {
            if hitbox_timer > 5
            {
                var destroy_effect = spawn_hit_fx(x + 35*spr_dir, y, player_id.nspecial_destroy_effect)
                destroy_effect.spr_dir = spr_dir
                destroy_effect.draw_xscale = draw_xscale
